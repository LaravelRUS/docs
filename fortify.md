...
**Вам необязательно использовать Fortify, чтобы использовать функционал аутентификации Laravel.** Вы всегда можете вручную взаимодействовать со службами аутентификации Laravel, следуя документации: [аутентификация](authentication), [сброс пароля](/docs/{{version}}/passwords), и [подтверждение по электронной почте](/docs/{{version}}/verification).
...
Чтобы начать включение двухфакторной аутентификации, ваше приложение должно сделать POST-запрос к URL `/user/two-factor-authentication`, определенному Fortify. Если запрос будет успешным, то пользователь будет перенаправлен обратно на предыдущий URL-адрес, а для переменной `status` сессии будет установлено значение `two-factor-authentication-enabled`. Вы можете использовать эту переменную `status` сессии в своих шаблонах, чтобы отобразить соответствующее сообщение об успешном выполнении. Если запрос был запросом XHR (аякс-запросом), будет возвращен `200` HTTP-ответ:

После выбора включения двухфакторной аутентификации пользователю все равно нужно "подтвердить" свою конфигурацию двухфакторной аутентификации, предоставив действительный код двухфакторной аутентификации. Поэтому ваше сообщение "успеха" должно информировать пользователя о том, что требуется подтверждение двухфакторной аутентификации:

```html
@if (session('status') == 'two-factor-authentication-enabled')
    <div class="mb-4 font-medium text-sm">
        Завершите настройку двухфакторной аутентификации ниже.
    </div>
@endif
```
...
Если вы создаете интерфейс на основе JavaScript, то вы можете сделать XHR GET-запрос (аякс-запрос) к URL `/user/two-factor-qr-code`, чтобы получить QR-код для двухфакторной аутентификации пользователя. Этот эндпоинт вернет объект JSON, содержащий ключ `svg`.

<a name="confirming-two-factor-authentication"></a>
#### Подтверждение двухфакторной аутентификации

Кроме отображения QR-кода двухфакторной аутентификации пользователя, вы должны предоставить текстовое поле, в которое пользователь может ввести действительный код аутентификации  для "подтверждения" своей конфигурации двухфакторной аутентификации. Этот код должен быть передан веб-приложению Laravel через POST-запрос к конечной точке `/user/confirmed-two-factor-authentication`, определенной Fortify.

Если запрос успешен, пользователь будет перенаправлен обратно на предыдущий URL, а переменная сессии `status` будет установлена в значение `two-factor-authentication-confirmed`:
...
Если запрос к конечной точке подтверждения двухфакторной аутентификации был выполнен через XHR (XMLHttpRequest), будет возвращен HTTP-ответ с кодом `200`.
...
Если вы создаете интерфейс на основе JavaScript, то вы можете сделать XHR GET-запрос (аякс-запрос) к URL `/user/two-factor-recovery-codes`. Этот эндпоинт вернет массив JSON, содержащий коды восстановления пользователя.

Чтобы повторно сгенерировать коды восстановления пользователя, ваше приложение должно сделать POST-запрос к URL `/user/two-factor-recovery-codes`.
...
Fortify позаботится об определении маршрута `/two-factor-challenge`, который возвращает этот шаблон. Ваш шаблон `two-factor-challenge` должен включать форму, которая делает POST-запрос к URL `/two-factor-challenge`. Действие `/two-factor-challenge` ожидает поле `code`, которое содержит действительный TOTP-токен, или поле `recovery_code`, которое содержит один из кодов восстановления пользователя.
...
Если запрос не был успешным, пользователь будет перенаправлен обратно к экрану двухфакторной проверки, и ошибки валидации будут доступны вам через общедоступную [переменную `$errors` шаблонов Blade](/docs/{{version}}/validation#quick-displaying-the-validation-errors). Или, в случае запроса XHR, ошибки валидации будут возвращены с `422` HTTP-ответом.
...
Fortify позаботится об определении маршрута `/register`, который возвращает этот шаблон. Ваш шаблон `register` должен включать форму, которая выполняет POST-запрос к URL `/register`, определенному в Fortify.
...
Если попытка регистрации будет успешной, то Fortify перенаправит вас на URI, настроенный с помощью параметра конфигурации `home` в файле конфигурации `fortify` вашего приложения. Если запрос был запросом XHR (аякс-запросом), будет возвращен `201` HTTP-ответ.
...
Fortify позаботится об определении маршрута `/forgot-password`, который возвращает этот шаблон. Ваш шаблон `forgot-password` должен включать форму, которая выполняет POST-запрос к URL `/forgot-password`, определенному в Fortify.
...
После перенаправления обратно к URL `/forgot-password` после успешного запроса переменная сессии `status` может использоваться для отображения состояния попытки запроса ссылки для сброса пароля. 

Значение этой переменной сессии будет соответствовать одной из строк перевода, определенных в [языковом файле](/docs/{{version}}/localization) `passwords` вашего приложения. Если вы хотите настроить это значение и еще не опубликовали языковые файлы Laravel, вы можете сделать это с помощью команды Artisan `lang:publish`:
...
Fortify позаботится об определении маршрута `/reset-password`, который возвращает этот шаблон. Ваш шаблон `reset-password` должен включать форму, которая выполняет POST-запрос к URL `/reset-password`, определенному в Fortify.
...
При желании вы можете добавить в шаблон вашего приложения `verify-email` кнопку, которая запускает POST-запрос к URL `/email/verification-notification`. Когда этот эндпоинт получает запрос, пользователю будет отправлена новая ссылка для подтверждения электронной почты, позволяющая пользователю получить новую ссылку для подтверждения, если предыдущая была случайно удалена или утеряна.
...
Этот шаблон отобразится при обращении к URL `/user/confirm-password`. Ваш шаблон `confirm-password` должен включать форму, которая выполняет POST-запрос к URL `/user/confirm-password`. Обработчик запроса к `/user/confirm-password` ожидает поле `password`, которое содержит текущий пароль пользователя.

Если пароль совпадает с текущим паролем пользователя, Fortify перенаправит пользователя на маршрут, к которому он пытался получить доступ. Если запрос был запросом XHR (аякс-запросом), будет возвращен `201` HTTP-ответ.
