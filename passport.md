...
При использовании этого метода аутентификации, по умолчанию создание JavaScript-модулей Laravel дает указание Axios всегда отправлять заголовки `X-CSRF-TOKEN` и` X-Requested-With`. Тем не менее, вам обязательно нужно включить свой CSRF-токен в [метатег HTML](/docs/{{version}}/csrf#csrf-x-csrf-token):
