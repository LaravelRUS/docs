...
Созданный контроллер содержит метод `getRemind`, который показывает форму для напоминания пароля. Вам надо создать эту форму во вьюхе `password.remind` (файл remind.blade.php в папке views/password - см. [view](/docs/responses#views)). Форма должна отправлять POST c email на метод `postRemind`
...
Laravel из коробки предлагает для аутентификации драйвера `database` и `eloquent`.  Чтобы узнать больше о том, как добавлять свои драйвера, изучите [документацию по расширению системы аутентификации](/docs/extending#authentication).