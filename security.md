...
Когда вызывается метод `attempt`, запускается [событие](/docs/4.2/events) `auth.attempt`. При успешной авторизации также запускается событие `auth.login`.
...
Для начала убедитесь, что ваша модель `User` реализует (implements) интерфейс `Illuminate\Auth\Reminders\RemindableInterface`. Модель `User`, которая идет с фреймворком, уже реализует его - она включает в себя `Illuminate\Auth\Reminders\RemindableTrait` с методами, которые реализуют этот интерфейс.
...
Созданный контроллер содержит метод `getRemind`, который показывает форму для напоминания пароля. Вам надо создать эту форму во вьюхе `password.remind` (файл remind.blade.php в папке views/password - см. [view](/docs/4.2/responses#views)). Форма должна отправлять POST c email на метод `postRemind`
...
Laravel из коробки предлагает для аутентификации драйвера `database` и `eloquent`.  Чтобы узнать больше о том, как добавлять свои драйвера, изучите [документацию по расширению системы аутентификации](/docs/4.2/extending#authentication).
