git 67fc3a87845824671a74c20bc54ad58a75012178
...
- [Условия запросов (Query Scopes)](#query-scopes)
    - [Глобальные](#global-scopes)
    - [Локальные](#local-scopes)
...
Если вы хотите вместе с моделью создать [миграцию](/docs/{{version}}/migrations) для соответственного изменения БД, добавьте опцию `--migration` или `-m`:
...
Кроме того, предволагается, что primary key является автоинкрементным полем, поэтому значение в нем фреймворк автоматически приводит к целому числу (int). Если вы хотите использовать строковые ключи, поставьте в вашей модели свойство `$incrementing` в `false`/
...
По умолчанию все Eloquent модели используют дефолтное соединение с базой данных. Вы можете указать имя нужного соединения в свойстве `$connection`:
...
После того, как вы создали модель [и таблицу, ассоциированную с ней](/docs/{{version}}/migrations#writing-migrations), можно попробовать получить из таблицы через модель какие-нибудь данные. Хороший путь - думать о модели как о продвинутом [конструкторе запросов](/docs/{{version}}/queries), при помощи которого можно писать более короткие и гибкие запросы, связанные с таблицей модели. Например:
...
В метод `find` также можно передавать массив значений ключей. В таком случае метод возвратит коллекцию.
...
Вы также можете использовать в запросе к моделям агрегирующие функции [конструктора запросов](/docs/{{version}}/queries) - такие, как `count`, `sum`, `max`. Они возвращают числовое значение, а не экземпляр модели.
...
И, конечно, в таблице модели должен существовать столбец `deleted_at`. Добавить его в [миграции](/docs/{{version}}/migrations) можно так:
...
## Условия запросов (Query Scopes)

<a name="global-scopes"></a>
### Глобальные условия

Глобальные условия запросов (global scopes) добавляют во **все** запросы для данной модели определённые условия. Например, механизм псевдоудаления моделей в Laravel построен на механизме глобальных условий - в можели с функцией псевдоудаления ставится условие отфильтровывать все псевдоудалённые значения. 

#### Написание глобальных условий

Определите класс, имплементирующий интерфейс `Illuminate\Database\Eloquent\Scope`. Этот интерфейс требует, чтобы в классе был всего один метод - `apply`, в котором вы можете написать нужную фильтрацию при помощи методов построителя запросов Laravel:
...
В структуре фреймворка нет какого-то специального места для классов глобальных условий, держите их там, где вам удобно.
...
Для того, чтобы запросы к модели начали фильтроваться при помощи глобальных запросов, переопределите в модели метод `boot` и используйте в нем метод `addGlobalScope`:
...
Теперь при запросе `User::all()` будет выполняться вот такой sql-запрос:

    select * from `users` where `age` > 200

#### Анонимные глобальные условия

Если вы не хотите создавать класс для применения глобальных условий в одной-единственной модели, вы можете определить глобальное условие прямо внутри метода `addGlobalScope`:
...
Первый аргумент в данном случае будет идентификатором этого глобального условия. Идентификатор нужен для того, чтобы быть способным отменить действие глобального условия:

    User::withoutGlobalScope('age')->get();

#### Нейтрализация глобального условия

Чтобы выполнить запрос без заданного глобального условия, используйте метод `withoutGlobalScope`:

    User::withoutGlobalScope(AgeScope::class)->get();

Можно отменить не только одно, но и несколько, или все глобальные условия - при помощи `withoutGlobalScopes`:
...
Локальные условия запросов (local scope, скоупы) позволяют записывать повторяющиеся длинные конструкции `where` в запросах моделей в виде короткого метода. Они определяются в модели в виде метода, начинающегося на scope.

Например, создадим условие `popular()` и `active()`:
...
#### Использование локальный условий Eloquent

Теперь мы можем использовать эти условия в запросе следующим образом:

    $users = App\User::popular()->active()->orderBy('created_at')->get();

#### Динамические условия

Локальные условия могут получать параметры. Для этого укажите их после первого аргумента `$query`:
