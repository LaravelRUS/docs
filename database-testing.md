...
Внутри замыкания последовательности вы можете получить доступ к свойствам `$index` или `$count` экземпляра последовательности, который вводится в замыкание. Свойство `$index` содержит количество итераций через последовательность, которые произошли до сих пор, тогда как свойство `$count` содержит общее количество раз, которое последовательность будет вызвана:
