...
```
Если вы хотите вызвать другую команду консоли и заглушить весь ее вывод, используйте метод `callSilent` . Метода `callSilent` имеет ту же сигнатуру, что и метод `call`:
