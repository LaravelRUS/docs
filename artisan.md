...
> Для лучшего пере использования кода, хорошая практика - сохранять ваши консольные команды легкими, а тяжелые операции делегировать  сервисам приложения.  Обратите внимание, что в приведенном ниже примере мы вводим сервисный класс, чтобы сделать "тяжелую" отправку сообщений.
...
Метод `anticipate` может использоваться для авто-дополнения возможного выбора. Пользователь может выбрать ответ, независимо от подсказок aвто-дополнения:
