---
git: 2aab9af23da9d663d781839fa236b2c92a117eb8
---
...
Дополнительно, способ хранения для channel `daily` может быть настроен с помощью опции `days`:
...
Если вы хотите добавить общую информацию между _всеми_ каналами, вы можете вызвать метод `Log::shareContext()`. Этот метод предоставит дополнительную информацию всем созданным каналам и всем каналам, которые будут созданы впоследствии. Обычно метод `shareContext` следует вызывать из метода `boot` поставщика услуг приложения:
