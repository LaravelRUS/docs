...
- [Контракты](#контракты)
  - [Введение](#введение)
    - [Контракты против Фасадов](#контракты-против-фасадов)
  - [Когда использовать контракты](#когда-использовать-контракты)
  - [Как использовать контракты](#как-использовать-контракты)
  - [Справочник контрактов](#справочник-контрактов)
...
[Фасады](facades) и глобальные помощники Laravel обеспечивают простой способ использования служб Laravel без объявления типов зависимости и извлечения их реализаций из контейнера служб. В большинстве случаев каждый фасад имеет эквивалентный контракт.
...
Многие типы классов в Laravel извлекаются из [контейнера служб](container), включая контроллеры, слушателей событий, посредники, очереди заданий и даже замыкания маршрутов. Итак, чтобы получить реализацию контракта, вы можете просто «объявить тип» интерфейса в конструкторе извлекаемого класса.
...
Когда слушатель события будет извлечен, контейнер служб, используя объявление типов в конструкторе класса, внедрит соответствующую зависимость. Чтобы узнать больше о регистрации в контейнере служб, ознакомьтесь с [его документацией](container).
