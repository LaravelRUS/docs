...
Каждый контракт имеет свою реализацию (implementation) во фреймворке. Например, есть реализация `Queue` с различными драйверами очередей и реализация `Mailer` с использованием [SwiftMailer](http://swiftmailer.org/).
...
Давайте рассмотрим пример кода с сильной связностью.
...
Этот класс тесно связан с реализацией кэширования `\SomePackage\Cache\Memcached`. Мы зависим и от способа кэширования (`memcached`), и от API данной библиотеки. Если мы хотим сменить кэширование с `memcached` на `redis`, нам придется вносить изменения в код класса `Repository`.
...
Вот таблица соответствий контрактов Laravel5 фасадам Laravel4 :
...
[Illuminate\Contracts\Foundation\Application](https://github.com/illuminate/contracts/blob/master/Foundatio&nbsp;pplication.php) | App
