...
Если группа маршрутов использует один и тот же [контроллер](/docs/{{version}}/controllers), вы можете использовать метод `controller` для определения общего контроллера всех маршрутов в группе. Затем при определении маршрутов вам нужно будет указать только метод вызываемого контроллера:
...
Как правило, неявная привязка модели не будет извлекать модели, которые были [удалены программно](/docs/{{version}}/eloquent#soft-deleting). Однако вы можете указать неявной привязке извлекать эти модели, привязав метод `withTrashed` к определению вашего маршрута:
...
PHP 8.1 ввёл поддержку [Enum (Перечислений)](https://www.php.net/manual/ru/language.enumerations.backed.php). Чтобы дополнить эту функцию, Laravel позволяет указывать [Enum с поддержкой строк](https://www.php.net/manual/ru/language.enumerations.backed.php) в определении маршрута, и Laravel будет вызывать маршрут только в том случае, если сегмент маршрута соответствует допустимому значению Enum. В противном случае автоматически будет возвращен ответ HTTP 404. Например, учитывая следующий Enum:
...
Как правило, ограничители частоты запросов определяются в методе `boot` класса `App\Providers\RouteServiceProvider` вашего приложения. Фактически, этот класс уже включает определение ограничителя частоты запросов, которое применяется к маршрутам в файле `routes/api.php` вашего приложения:
