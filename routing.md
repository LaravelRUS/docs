...
Вы можете определить соответствие параметра пути регулярному выражению глобально для всех роутов. Паттерны задаются в методе `boot` сервис-провайдера `RouteServiceProvider`. Например, если у вас `{id}` в урлах это всегда числовое значение:
