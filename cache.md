...
Если элемента нет в кэше, будет выполнена переданная в метод `remember` функция, а её результат будет помещён в кэш.

Метод `rememberForever` запросит данные из кэша, а если их там нет, положит их туда навечно:
