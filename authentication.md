...
Так же удостоверьтесь, что таблица `users` (или аналогичная) содержит обнуляемое (nullable) поле `remember_token` длиной в 100 символов.
...
Так же для этого в таблице `users` должно быть строковое поле `remember_token`, используемое для хранения идентификатора (токена).
