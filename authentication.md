git 8ecab36e29082f0c8b96be1493e1fa9570ee75cb
...
В Laravel уже включены все необходимые миграции для аутентификации, но при самостоятельном создании схемы БД не забудьте про два обязательных
поля в таблице `user` (или аналогичной):
...
Метод `attempt` принимает массив «ключ-значение» в качестве первого аргумента. Значения ключа `password` будет [прохэшировано](/docs/5.0/hashing).
...
При вызове метода `attempt` генерируется [событие](/docs/5.0/events) `auth.attempt`. Если аутентификация прошла успешно
...
И наконец, можно использовать мощь [сервис-контейнера](/docs/5.0/container), указав в качестве аргумента в конструкторе
...
Вы можете ипользовать [посредников](/docs/5.0/middleware) для ограничения доступа к роутам. В Laravel уже есть посредник `auth`,
...
для аутентификации черех API. Для этого нужно создать [посредника](/docs/5.0/middleware), который будет вызывать метод `onceBasic`:
