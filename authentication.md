git 1eb1c74bc7481a96f965de5f6f4e2e7ab68e1a41
...
> {tip} **Хотите быстро начать работу?** Установите composer-пакет `laravel/ui` и запустите `php artisan ui vue --auth` в свежем приложении Laravel. Запустите миграцию базу данных и зайдите на `http://your-app.test/register`. У вас появилась готовая система аутентификации, которую вы можете менять под себя как захотите!
...
> {note} `logoutOtherDevices` работает только если в [группу роутов](/docs/{{version}}/middleware#middleware-groups) `web` в `app/Http/Kernel.php` добавлен посредник `Illuminate\Session\Middleware\AuthenticateSession::class`.   
...
> {note} Когда вызван метод `logoutOtherDevices`, все сессии кроме текущей будут инвалидированы. Пользователь окажется незалогиненным на сайте.
