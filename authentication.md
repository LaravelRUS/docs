...
> {tip} **Хотите быстро начать работу?** Установите composer-пакет `laravel/ui` и запустите `php artisan ui vue --auth` в свежем приложении Laravel. Запустите миграцию базу данных и зайдите на `http://your-app.test/register`. У вас gjzdbkfcm готовая система аутентификации, которую вы можете менять под себя как захотите!
