...
В этом примере класс `OrderRepository` автоматически встроится в контроллер. Это значит, что при использовании [юнит-тестов](/docs/testing) класс-заглушка ("mock") для `OrderRepository` может быть добавлен в контейнер, таким образом легко имитируя взаимодействие с БД.

#### Другие примеры использования IoC

[Фильтры](/docs/routing#route-filters), [вью-композеры](/docs/responses#view-composers) и [обработчики событий](/docs/events#using-classes-as-listeners) могут также извлекаться из IoC. При регистрации этих объектов просто передайте имя класса, который должен быть использован:
