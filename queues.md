...
> {note} Возможности Redis `serializer` и `compression` не поддерживаются драйвером очереди `redis`.
...
> {note} Поскольку функции-замыкания сериализуются и выполняются позже в очереди Laravel, вам не следует использовать $this в замыканиях.
