git 86ab190845fd30951808104ec5185ae94355c529
...
После установки вам надо открыть для записи папку `app/storage` со всеми подпапками. См. секцию [Установка](/docs/4.1/installation).
...
Этот маршрут сообщает Laravel, что запросы к `/users` должны вызывать метод `getIndex` класса `UserController`. Для дополнительной информации см. [раздел о контроллерах](/docs/4.1/controllers).
...
Для создания миграции мы воспользуемся утилитой командной строки [Artisan](/docs/4.1/artisan). Выполните следующую команду в корневой папке вашего проекта:
