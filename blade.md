git bfb3ba93056348d8112bc3a2ee1cde6f80e1bd55
...
> **Примечание:** Конструкция `{{ }}` автоматически применяет к содержимому PHP-функцию `htmlentities` для предотвращения XSS-атак.
...
По умолчанию конструкция `{{ }}` применяет к содержимому PHP-функцию `htmlentities` для предотвращения XSS-атак. Если вам требуется вывести текст без экранирования, используйте следующий синтаксис:
...
Вы так же можете проверить наличие какого-либо контента в секции используя директиву `@hasSection`:
...
Директива `@each` комбинирует в себе директивы цикла и включения страниц:

    @each('view.name', $jobs, 'job')

Первый аргумент - название шаблона, который отображает элемент коллекции. Второй - собственно массив или коллекция, элементы которой будут отображаться. Третий - название переменной, которая в шаблоне представляет собой элемент коллекции. Так, например, если требуется отобразить массив `jobs`, обычно, вы захотите получить каждый элемент этого массива в переменную `job` внутри вашего шаблона. Ключ текущей итерации доступен как переменная `key`. 
...
Казалось бы, зачем здесь использовать шаблонизатор, если можно просто написать `<!-- -->` ? Но фишка в том, в отличие от HTML-комментариев, Blade-комментарии никогда не попадут в результирующий HTML.
...
Blade позволяет создавать свои директивы. Для этого нужно использовать `Blade::directive()` в методе boot() сервис-провайдера. В аргументах - название директивы и функция-замыкание, которая возвращает конструкцию для вставки в шаблон.

Например, создадим директиву `@datetime($var)`, которая будет форматировать дату (объект Carbon) определённым, нужным нам образом.
