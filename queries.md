---
git: 8138b3906edb32f9805a6674b6c00fc38b699b4a
---
...
Метод `whereBetweenColumns` проверяет, что значение столбца находится между двумя значениями двух столбцов в одной и той же строке таблицы:
...
Метод `whereNotBetweenColumns` проверяет, что значение столбца находится вне двух значений двух столбцов в одной и той же строке таблицы:
...
> **Warning**  
> Полнотекстовый поиск поддерживаются в настоящее время для MySQL и PostgreSQL.

Методы `whereFullText` и `orWhereFullText` позволяют добавлять полнотекстовые "условия" в запрос для столбцов, имеющих [полнотекстовые индексы](/docs/{{version}}/migrations#available-index-types). Laravel автоматически преобразует эти методы в соответствующий SQL-код для используемой базы данных. Например, для приложений, использующих MySQL, будет сгенерировано условие `MATCH AGAINST`:
