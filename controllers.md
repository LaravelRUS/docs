...
Если вы хотите создавать ссылки на «действие» контроллера, используя относительные имена имена классов, то сначала нужно зарегистрировать «корневое» пространство имён с контроллерами:
...
Ресурс-контроллеры упрощают построение RESTful контроллеров, работающих с ресурсами. Например, вы можете создать контроллер, обрабатывающий фотографии, хранимые вашим приложением. Вы можете быстро создать такой контроллер, используя Artisan-команду `controller:make`.
...
> **Примечание:** Внедрение зависимостей в методы контроллера полностью дружат с механизмом [связанных моделей](/docs/master/routing#route-model-binding).
