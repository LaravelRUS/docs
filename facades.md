...
Фасады предоставляют «статический» интерфейс (`Foo::bar()`) к классам, доступным в [сервис-контейнере](/docs/{{version}}/{{version}}/container). Laravel поставляется
...
> **Примечание:** перед погружением в фасады настоятельно рекомендуется как можно детальнее изучить [сервис-контейнер Laravel](/docs/{{version}}/{{version}}/container).
...
Самое лучшее место для регистрации этой связки - новый [сервис-провайдер](/docs/{{version}}/{{version}}/container#service-providers) который мы назовём
...
по которой фасады вообще существуют. Эта тема подробнее раскрыта в соответствующем разделе документации - [фасады-заглушки](/docs/{{version}}/{{version}}/testing#mocking-facades).
