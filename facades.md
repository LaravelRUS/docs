...
Фасады предоставляют «статический» интерфейс (`Foo::bar()`) к классам, доступным в [IoC-контейнере](/docs/{{version}}/ioc). Laravel поставляется
...
> **Примечание:** перед погружением в фасады настоятельно рекомендуется как можно детальнее изучить [IoC-контейнер Laravel](/docs/{{version}}/ioc).
...
Самое лучшее место для регистрации этой связки - новый [сервис-провайдер](/docs/{{version}}/ioc#service-providers) который мы назовём
