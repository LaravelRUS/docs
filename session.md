...
Laravel использует имя ключа `flash` для внутренний целей, поэтому вы не дожны добавлять в сессию данные с таким ключом.
...
> **Примечание:** драйвер `array` обычно используется для [юнит-тестов](/docs/master/testing), так как он на самом деле не сохраняет данные для последующих запросов.
