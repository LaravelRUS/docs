...
Для этого вы должны определить почтовую программу в файле конфигурации вашего приложения `mail`, который использует транспорт`failover`. Массив конфигурации для почтовой программы `failover` вашего приложения должен содержать массив `mailers`, который определяет очередность, в котором почтовые драйверы должны быть выбраны для доставки:
...
Наконец, вы можете указать глобальный адрес «кому», вызвав alwaysToметод, предлагаемый фасадом Mail. Как правило, этот метод следует вызывать из bootметода одного из сервис-провайдеров вашего приложения:
